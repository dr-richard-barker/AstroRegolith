...
def main() -> int:
    ...
    if not args.check:
        # No "@" anywhere in this header: BibTeX scans for it outside entries too,
        # and a mention of an entry type in a comment line makes it stop with
        # "I was expecting a `{' or a `('".
        header = (
            "% references.bib — generated by scripts/check_references.py\n"
            "%\n"
            "% Every journal entry below is written from metadata returned by the Crossref\n"
            "% API for its DOI, not from memory. Re-run the script to refresh or verify.\n"
            "% The misc entries have no DOI and must be checked by hand; their URLs are\n"
            "% in the NON_DOI table of that script.\n\n"
        )
