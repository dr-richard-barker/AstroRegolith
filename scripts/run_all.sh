...
#
# Steps 00-02 and 10 hit the network (GitHub raw, NASA OSDR, NASA PSI, EBI
# QuickGO) and are idempotent — a file already present is not re-downloaded.
# Steps 03-09 are pure functions of data/, so their outputs are byte-stable
# across re-runs. Step 10 runs with the fetch group because it downloads and
# re-encodes the OSD-670 imagery; its output is committed, so a SKIP_FETCH run
# reuses public/images/ as-is.
set -euo pipefail
cd "$(dirname "$0")/.."
